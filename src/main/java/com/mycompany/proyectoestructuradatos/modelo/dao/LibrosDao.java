...
//                psInsert.setDate(3, fechaSql);
    ...
    public List<Map<String, Object>> consultarLibroPorId(int idLibro) {
        List<Map<String, Object>> libro = new ArrayList<>();
        String query = "SELECT id_libro, titulo, autor, disponible FROM public.libros WHERE id_libro = ?";
    ...
    public boolean devolucionLibro(int idLibro, int documentoUsuario) {
        String updateLibroQuery = "UPDATE public.libros SET disponible = TRUE WHERE id_libro = ?";
        String updatePrestamoQuery = "UPDATE public.prestamos SET estado_prestamo = FALSE , fecha_devolucion_real = ? WHERE id_libro = ? AND id_usuario = ? ";
        ...
        try (PreparedStatement psUpdateLibro = conexion.prepareStatement(updateLibroQuery);
            ...
            psUpdatePrestamo.setInt(2, idLibro);
            psUpdatePrestamo.setInt(3, documentoUsuario);
